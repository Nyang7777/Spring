							...
							<tr>
								...
								<c:choose>
									<c:when test="${k.status == 1}">
										<td style="color: gray">삭제된 게시물</td>
									</c:when>
									<c:otherwise>
					...
					<td colspan="4">
						<ol class="paging">
							...
							<c:choose>
								...
								<c:otherwise>
									<li><a href="/bbs.do?cPage=${paging.beginBlock-paging.pagePerBlock }">이전으로</a></li>
								</c:otherwise>
							</c:choose>
							...
							<c:forEach begin="${paging.beginBlock }" end="${paging.endBlock }" step="1" var="k">
								...
								<c:if test="${ k != paging.nowPage}">
									<li><a href="/bbs.do?cPage=${k}">${k}</a></li>
								</c:if>   		
							</c:forEach>
							...
							<c:choose>
								...
								<c:otherwise>
									<li><a href="/bbs.do?cPage=${paging.beginBlock+paging.pagePerBlock }">다음으로</a></li>
								</c:otherwise>
							</c:choose>
						</ol>
					</td>
